...
INSERT INTO service_slot (start, duration, repeat_start, repeat_interval, service_id)
SELECT
  start_time_unix AS start,
  5400000 AS duration, -- 1.5 hours in milliseconds
  repeat_start_unix AS repeat_start,
...
FROM (
  ...
  FROM (
    SELECT
      ...
      s.id as service_id,
      staff_handle,
      date::timestamptz AS start_time_unix,
      date::timestamptz AS repeat_start_unix
    FROM schedule_data
    JOIN service s ON s.title = service_title
    GROUP BY date, staff_handle, service_id, start_time_unix, repeat_start_unix
  ) AS grouped_schedule
) AS final_schedule;
